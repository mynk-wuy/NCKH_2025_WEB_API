...
document.addEventListener("DOMContentLoaded",  async function () {
    ...
    // Cập nhật dữ liệu dự đoán mỗi phút
    // setInterval(fetchPredictions, 60 * 1000);
    // Set up regular updates every minute
    setInterval(async () => {
        console.log('Fetching new predictions...'); // Debug log
...
function fetchSensorData(startTime = null, endTime = null) {
    ...
    const apiUrl = `http://localhost/routes/giamsat1.php?start_time=${encodeURIComponent(tenMinutesAgoStr)}&end_time=${encodeURIComponent(currentTimeStr)}`;
        ...
        .then(data => {
            ...
            sortedData.forEach(entry => {
                try {
                    ...
                    if (entry.sensor_id === "A4002CO") {
                        coValues.push(value);
                        timeLabels.push(timeStr);
                    } else if (entry.sensor_id === "A4002H2") {
                        h2Values.push(value);
                    } else if (entry.sensor_id === "A4002NH3") {
    ...
    try {
        ...
        if (coElement) {
            ...
            if (coValues.length > 0) {
                ...
                coElement.style.color = value > 800 ? "red" : value > 500 ? "orange" : "black";
            }
        }
        ...
        if (h2Element) {
            ...
            if (h2Values.length > 0) {
                ...
                h2Element.style.color = value > 800 ? "red" : value > 500 ? "orange" : "black";
            }
        }
        ...
        if (nh3Element) {
            ...
            if (nh3Values.length > 0) {
                ...
                nh3Element.style.color = value > 800 ? "red" : value > 500 ? "orange" : "black";
            }
        }
    } catch (e) {
...
function updateStats() {
    try {
        ...
        const apiUrl = `http://localhost/routes/giamsat1.php?start_time=${encodeURIComponent(startTimeStr)}&end_time=${encodeURIComponent(endTimeStr)}`;
        fetch(apiUrl)
            .then(response => {
            ...
            .then(data => {
                ...
                data.forEach(entry => {
                    ...
                    if (entry.sensor_id === "A4002CO") dailyData.co.push(value);
                    else if (entry.sensor_id === "A4002H2") dailyData.h2.push(value);
                    else if (entry.sensor_id === "A4002NH3") dailyData.nh3.push(value);
                });
...
function updatePredictionChart(apiData) {
    try {
        ...
        const { A4002CO, A4002H2, A4002NH3 } = apiData.predictions;
        
        // Kiểm tra dữ liệu
        if (!A4002CO || !A4002H2 || !A4002NH3 || 
            A4002CO.length === 0 || A4002H2.length === 0 || A4002NH3.length === 0) {
            console.error('Thiếu dữ liệu cảm biến');
            return;
        }

        // Chuẩn bị dữ liệu
        const labels = A4002CO.map(item => {
            const date = new Date(item.timestamp);
            return date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
        });

        const coData = A4002CO.map(item => parseFloat(item.predicted_value));
        const h2Data = A4002H2.map(item => parseFloat(item.predicted_value));
        const nh3Data = A4002NH3.map(item => parseFloat(item.predicted_value));
