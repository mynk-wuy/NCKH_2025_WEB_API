...
#define CO_THRESHOLD    8000    // Ngưỡng CO nguy hiểm (ppm)
#define H2_THRESHOLD    8000   // Ngưỡng H2 nguy hiểm (ppm)
#define NH3_THRESHOLD   8000    // Ngưỡng NH3 nguy hiểm (ppm)
...
// Các topic MQTT
const char* fanControlTopic = "A4002/fan/control";
const char* ledControlTopic = "A4002/led/control";
const char* fanStatusTopic = "A4002/fan/status";
const char* ledStatusTopic = "A4002/led/status";
const char* sensorDataTopic = "A4002/sensors/data"; // Topic cho dữ liệu cảm biến
const char* modeTopic ="A4002/mode/control";
...
void loop() {
  ...
  if (currentMillis - lastSensorReadTime >= SENSOR_READ_INTERVAL || timerFlag) {
    ...
    float h2PPM = map(h2Raw, 0, 4095, 10, 1000);
    float nh3PPM = map(nh3Raw, 0, 4095, 10, 1000);  
    //giá trị cảm bien map chua loc
    Serial.println("Đọc giá trị cảm biến chưa lọc:");
...
void publishSensorData(float co, float h2, float nh3) {
  ...
  String jsonPayload = "{\"A4002CO\":" + String(co) + 
                       ",\"A4002H2\":" + String(h2) + 
                       ",\"A4002NH3\":" + String(nh3) + "}";
